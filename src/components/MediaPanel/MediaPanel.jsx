...
import {
  LayoutGrid, Image as ImageIcon, Video, Newspaper, Bookmark, Link as LinkIcon, History as HistoryIcon, Settings, Trash2, Moon, Sun, Zap, Wifi, ShieldOff
} from 'lucide-react';
...
export const MediaPanel = () => {
  const { activeCategory, images, videos, news, history, removeHistoryItem, clearHistory, isOffline, setIsOffline, executeSearch, results } = useSearch();
      ...
      <div className="panel-scroll-area">
        <AnimatePresence mode="wait">
                ...
                bookmarks.map(site => (
                  <div key={site.id || site.url} className="saved-card">
                    <div className="saved-card-header">
                      <img src={site.favicon || `https://www.google.com/s2/favicons?domain=${site.domain}&sz=32`} alt="" className="result-favicon" />
          ...
          {activeCategory === 'History' && (
            <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="history-list-container">
              {history.length === 0 ? (
                <div className="empty-media-msg">No search history yet.</div>
              ) : (
                <>
                  ...
                  {history.map((q, idx) => (
                    <div key={idx} className="history-item-row">
                      <span className="history-item-text" onClick={() => executeSearch(q)}>
                        🔍 {q}
                      </span>
                      <button type="button" className="history-item-del" onClick={() => removeHistoryItem(q)}>
                        &times;
                      </button>
                    </div>
                  ))}
