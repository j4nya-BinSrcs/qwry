...
import { Sparkles, Send, X, Grid, MessageSquare } from 'lucide-react';
...
export const AIChat = () => {
  const { query, aiOverview, executeSearch } = useSearch();
  ...
  const handleSendChat = async (e) => {
    ...
    try {
      const res = await searchApi.aiChat(currentInput, chatContextItems);
      ...
      <div className="panel-scroll-area ai-chat-scroll">
        ...
        >
                ...
                {chatContextItems.map(item => (
                  ...
                  >
                    <span className="dropped-card-title">
                      {item.type === 'image' ? '🖼️ Image' : `🔗 ${item.title || item.domain}`}
                    </span>
                    <button
                      type="button"
          ...
          <form onSubmit={handleSendChat} className="ai-chat-form">
            <input
              ...
              placeholder={isOver ? "Drop to add context ✨" : "Ask a follow up... (drop here)"}
              value={chatInput}
        ...
        <div className="related-suggestions-section">
          ...
          <div className="suggestions-pill-list">
            {(aiOverview?.suggestions || ['glassmorphism css', 'dark mode palette', 'react animations']).map(pillText => (
