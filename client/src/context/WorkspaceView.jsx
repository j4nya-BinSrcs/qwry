...
function renderSources(items, loading, sessionId) {
  const { setNodeRef, isOver } = useDroppable({ id: "workspace-drop", data: { type: "workspace" } });

  if (loading && items.length === 0) return <Spinner />;
  if (!loading && items.length === 0) {
    return (
      <EmptyState icon={Layers} message="Drag search results here to save them for later" />
    );
  }
  return (
    <div ref={setNodeRef}
...
function renderSummaries(items) {
  ...
  if (hasSummaries.length === 0) {
    return <EmptyState icon={Sparkles} message="No items with summaries yet" />;
  }
  return (
    <div className="space-y-1 p-2">
      {hasSummaries.map((item) => (
        <div key={item.id} className="bg-white border border-border rounded-md px-3 py-2.5">
...
function renderReadingQueue(reads, sessionId, wsId) {
  ...
  if (reads.length === 0) {
    return <EmptyState icon={Book} message="No items in reading queue" />;
  }
  return (
    <div className="space-y-1 p-2">
      {reads.map((r) => (
        <div key={r.id} className="flex items-center gap-3 bg-white border border-border rounded-md px-3 py-2">
...
function renderNotes(notes, sessionId, wsId) {
    ...
    <div className="flex flex-col h-full">
      <div className="shrink-0 px-3 py-2 border-b border-border space-y-2">
        ...
        <button onClick={handleAdd} disabled={!title.trim()}
          className="flex items-center gap-1 text-xs px-2.5 py-1 rounded bg-text text-white hover:opacity-80 transition-opacity disabled:opacity-30"
        ><Plus size={12} /> Add Note</button>
      </div>
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {notes.length === 0 && <EmptyState icon={FileText} message="No notes yet" />}
        {notes.map((n) => (
          <div key={n.id} className="bg-white border border-border rounded-md px-3 py-2">
            {editingId === n.id ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-1">
                  <button onClick={() => handleUpdate(n.id)}
                    className="text-xs px-2 py-0.5 rounded bg-text text-white hover:opacity-80 transition-opacity"
...
function renderAITools(sessionId, wsId, activeWs, tags, items) {
    ...
    <div className="p-3 space-y-3">
      <div className="bg-white border border-border rounded-md p-3">
        ...
        <button onClick={() => setChatOpen(true)}
          className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded bg-text text-white hover:opacity-80 transition-opacity"
        ...
        {selectedItemId && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <button onClick={async () => {
              ...
              className="text-xs px-2 py-1 rounded bg-text text-white hover:opacity-80 transition-opacity"
            >Assign</button>
          </div>
        )}
      </div>
...
function renderPins(pins, sessionId, wsId) {
  const deletePin = useWorkspaceStationStore((s) => s.deletePin);

  if (pins.length === 0) {
    return <EmptyState icon={Pin} message="No pinned items" />;
  }
  return (
    <div className="space-y-1 p-2">
      {pins.map((p) => (
        <div key={p.id} className="flex items-center gap-3 bg-white border border-border rounded-md px-3 py-2">
...
function renderTimeline(events) {
  ...
  return (
    <div className="space-y-1 p-2">
      {events.map((e) => (
        <div key={e.id} className="flex items-start gap-3 bg-white border border-border rounded-md px-3 py-2">
...
function renderTags(tags, sessionId, wsId, items) {
  ...
  return (
    <div className="p-2 space-y-2">
      <div className="bg-white border border-border rounded-md p-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={async () => {
            ...
            className="flex items-center gap-1 text-xs px-2.5 py-1 rounded bg-text text-white hover:opacity-80 transition-opacity"
      ...
      <div className="space-y-1">
        ...
        {tags.map((t) => (
          <div key={t.id} className="flex items-center gap-2 bg-white border border-border rounded-md px-3 py-2">
...
function renderImages(images, sessionId, wsId) {
  const deleteImage = useWorkspaceStationStore((s) => s.deleteImage);

  if (images.length === 0) {
    return <EmptyState icon={Image} message="No images saved" />;
  }
  return (
    <div className="grid grid-cols-3 gap-2 p-2">
      {images.map((img) => (
        <div key={img.id} className="group relative bg-white border border-border rounded-md overflow-hidden">
...
function renderVideos(videos, sessionId, wsId) {
  const deleteVideo = useWorkspaceStationStore((s) => s.deleteVideo);

  if (videos.length === 0) {
    return <EmptyState icon={Video} message="No videos saved" />;
  }
  return (
    <div className="space-y-1 p-2">
      {videos.map((v) => (
        <div key={v.id} className="flex items-start gap-3 bg-white border border-border rounded-md px-3 py-2">
          {v.thumbnail ? (
...
function renderComparisons(comparisons, sessionId, wsId) {
  const deleteComparison = useWorkspaceStationStore((s) => s.deleteComparison);
  const [expandedId, setExpandedId] = useState(null);

  if (comparisons.length === 0) {
    return <EmptyState icon={ListOrdered} message="No comparisons yet" />;
  }
  return (
    <div className="space-y-1 p-2">
      {comparisons.map((c) => (
        <div key={c.id} className="bg-white border border-border rounded-md px-3 py-2">
...
function renderSearch(sessionId, wsId) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        {results.length > 0 && results.map((r, i) => (
          <div key={`${r.object_id}-${i}`} className="bg-white border border-border rounded-md px-3 py-2">
            <div className="flex items-center gap-2">
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 text-center">
        ...
        <button onClick={() => { const name = prompt("Workspace name:"); if (name) createWorkspace(sessionId, name); }}
          className="text-xs px-3 py-1.5 rounded bg-text text-white hover:opacity-80 transition-opacity"
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {activeSection === "sources" && renderSources(items, loading, sessionId)}
        {activeSection === "summaries" && renderSummaries(items)}
        {activeSection === "reading" && renderReadingQueue(station.reads, sessionId, activeId)}
        {activeSection === "notes" && renderNotes(station.notes, sessionId, activeId)}
        {activeSection === "ai-tools" && renderAITools(sessionId, activeId, activeWs, station.tags, items)}
        {activeSection === "pins" && renderPins(station.pins, sessionId, activeId)}
        {activeSection === "timeline" && renderTimeline(station.timeline)}
        {activeSection === "tags" && renderTags(station.tags, sessionId, activeId, items)}
        {activeSection === "images" && renderImages(station.images, sessionId, activeId)}
        {activeSection === "videos" && renderVideos(station.videos, sessionId, activeId)}
        {activeSection === "comparisons" && renderComparisons(station.comparisons, sessionId, activeId)}
        {activeSection === "search" && renderSearch(sessionId, activeId)}
      </div>
    </div>
  );
}
