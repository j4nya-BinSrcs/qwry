...
void main() {
  ...
  if (uEnableMouse > 0.5) {
    pinchX = mix(pinchX, uMouse.x, clamp(uMouseStrength * 1.5, 0.0, 1.0) * uMouseActive);
  }
  ...
  if (uEnableMouse > 0.5) {
    vec2 md = uv - uMouse;
    float d2 = dot(md, md);
    bright += clamp(uMouseStrength, 0.0, 1.0) * uMouseActive * exp(-d2 * 4.0) * 1.5;
  }
  col *= bright;
...
const WebThreads = ({
  ...
  useEffect(() => {
    ...
    const loop = (t) => {
      ...
      currentActive += 0.15 * (targetActive - currentActive);
